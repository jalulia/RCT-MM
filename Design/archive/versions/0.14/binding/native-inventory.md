...
All paths below are relative to the external `validation_package/` source collection. File labels are inventory identifiers, not original exhibit numbers. `local` provenance applies throughout. The source files remain outside this publication. This inventory preserves file identity and exact locators; a listed path is not a download link.
...
**Return date.** CASE-01 p.31 dates the CP return October 21; the binding packet preserves that printed claim. B1 and FN/FM instead distinguish October 21 funding/debit from an October 22 return posting. The return description refers back to October 21. The game must preserve the dates by source and event type until the original statement and payment lifecycle resolve them. A QBO payment date of October 20 is a third kind of date, not an interchangeable timestamp.
...
The source tree and relevant immediate parent were inventoried without executing their code. Exact-filename searches were also run within the local document and shared-project collections for `Madwell_BofA Statements_2024.pdf`, `20241031-statements-6976-.pdf`, and `2 Bank of America_Madwell LLC_February 2019.pdf`; no matches were returned. This is a bounded filename search, not proof the originals do not exist under other names, in archives, or elsewhere. IX is an inventory aid, not an original-source substitute. Its relevant missing objects were not recovered.

For a bounded historical comparison, obtain the cited original BoA pages and explicit return-to-payment linkage; establish a later observation of the same BILL object with dated provenance; and bind an ordinary control. Preserve hashes and exact locators. The October 23 direction conflict must be resolved or shown as unresolved without a directional cash animation. The full confirmation links the Chase description to the BILL payment; it does not settle direction or final receipt.
