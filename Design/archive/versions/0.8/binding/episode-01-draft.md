...
Following the separate native-corpus inventory, this packet's author independently read the exact CSV records listed below. Those are inspected **exports and statement extractions**, not authenticated original statements, invoices, audit logs or recipient confirmations. The source's labels “proved,” “closed” and “false state” remain attributed conclusions.

File paths below are relative to `/Users/juliacompton/Documents/Ø/The Madwell Architecture/validation_package`. CSV record numbering counts the header as record 1; physical line differences are explicit. Full file hashes, provenance and discovery status are collected in the companion [native inventory](native-inventory.md). An uninspected original is not an original known to be absent.
