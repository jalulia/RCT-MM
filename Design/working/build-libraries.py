...
def shell(title,body,location):
 ...
 return '''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>'''+esc(title)+''' — Mad Money Tycoon</title><style>'''+css+'''</style></head><body><header><a href="../design-review.html">Mad Money Tycoon</a>'''+nav+'''</header><main>'''+body+'''</main><script src="../assets/sprite-catalogue/motion-player.js"></script><script>const fit=new ResizeObserver(entries=>entries.forEach(({target:t})=>{const b=JSON.parse(t.dataset.bounds),s=JSON.parse(t.dataset.size),k=Math.max(1,Math.min(6,Math.floor((t.clientWidth-35)/b.width),Math.floor((t.clientHeight-35)/b.height)));Object.assign(t.querySelector('img').style,{width:s[0]*k+'px',height:s[1]*k+'px',left:Math.round((t.clientWidth-b.width*k)/2-b.x*k)+'px',top:Math.round((t.clientHeight-b.height*k)/2-b.y*k)+'px'})}));document.querySelectorAll('[data-bounds]').forEach(t=>fit.observe(t));</script></body></html>'''
...
for o in cat['objects']:
 d=o['display'];base='../assets/sprite-catalogue/'
 entries.append({'id':'object-'+o['id'],'title':o['name'],'png':base+d['poster'],'gif':base+d['gif'],'kind':'animation' if d['animated'] else 'still','group':'Motion' if d['animated'] else 'Components','category':o['group'],'size':[d['width'],d['height']],'bounds':d['bounds'],'route':'#art/kit-'+o['id'],'object':o['id'],'caption':d['label']+'. '+o['description']})
for directory,stems in [('office-study',['shell','interior','parts']),('art-system',['stage0','stage1','stage2','stage4'])]:
 for stem in stems:
  f=root/'assets'/directory/(stem+'.png');target=preview/(directory+'-'+stem+'.gif')
  with Image.open(f) as im:
   im.convert('RGB').quantize(colors=256,method=Image.Quantize.MEDIANCUT).save(target);size=list(im.size)
  names={'shell':'Porter / shell study','interior':'Porter / interior study','parts':'Architectural parts','stage0':'Porter / footprint','stage1':'Porter / shell','stage2':'Porter / contents','stage4':'Porter / roof'}
  entries.append({'id':directory+'-'+stem,'title':names[stem],'png':rel(f,preview),'gif':target.name,'kind':'still','group':'Architecture','size':size,'route':'#art/architecture-study' if directory=='office-study' else '#art/assembly','caption':'Architectural state · still image. The assembled park uses these forms.'})
...
def preview_card(e):
 ...
 if e.get('object'):links+=f'<a href="../assets/sprite-catalogue/index.html#{e["object"]}">Object & all frames ↗</a>'
 if e['kind']=='animation':links='<button data-preview-motion aria-pressed="true">Pause previews</button>'+links
 motion=f' data-motion-src="{esc(e["gif"])}"' if e['kind']=='animation' else ''
 bounds=f' data-bounds="{esc(json.dumps(e["bounds"]))}" data-size="{esc(json.dumps(e["size"]))}"' if e.get('object') else ''
 return f'<figure class="card {"photo" if e.get("group") in ["Documents","Episode"] else ""}"><div class="image"{bounds}><img src="{esc(e["png"])}"{motion} alt="{esc(e["title"])}" loading="lazy"></div><figcaption class="copy"><span class="meta">{e["size"][0]} × {e["size"][1]} / {esc(e["kind"].upper())}</span><h3>{esc(e["title"])}</h3><p>{esc(e["caption"])}</p><div class="links">{links}</div></figcaption></figure>'
body='<div class="intro"><div><span class="status">CURRENT / ART '+project['artRevision']+'</span><h1>Preview library</h1><p>Scenes, motion, architecture and document materials. Visible animations play automatically. Every preview has a GIF download; static states stay labelled as stills.</p></div><aside><b>Need an individual object?</b><p>The sprite catalogue contains all 44 objects, 136 frame slots, transparent sheets and labelled animation sequences.</p><a href="../assets/sprite-catalogue/index.html">Open the object catalogue ↗</a></aside></div><nav class="jump"><button data-preview-motion aria-pressed="true">Pause previews</button>'+''.join(f'<a href="#{s.lower()}">{s}</a>' for s in ['Episode','Scenes','Motion','Architecture','Components','Documents'])+'</nav>'
