...
Object.assign(figures,artFigures,officeFigures,bindingFigures,parkFigures,materialFigures,editorialFigures,episodeFigures);
...
for(const p of pages){
 ...
 html=html.replace(/<p>(<img[^>]+>)<\/p>/g,(_,img)=>{
  ...
  return `<figure class="inline-example"><a href="${href}" ${animated?'class="inline-object-motion"':''}>${animated?objectMedia(object,{title:alt,width:390,height:245}):img}</a><figcaption>${alt}<a href="${href}">${object?'Inspect '+object:catalogue?'Inspect catalogue':'Open source image'} ↗</a>${animated?playbackButton:''}</figcaption></figure>`;
 });
