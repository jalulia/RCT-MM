...
import { readingFigures,readingLayout } from './reading-figures.mjs';
import { figures,views,productionPlan } from './figures.mjs';
import { catalogue, objectMedia, playbackButton } from './object-media.mjs';
...
import { episodeFigures } from './episode-figures.mjs';
import { materialFigures } from './document-materials.mjs';
Object.assign(figures,artFigures,officeFigures,bindingFigures,parkFigures,materialFigures,editorialFigures,episodeFigures,readingFigures);
...
const names=[['game','Game','Player role and the two structures to test.'],['episode','First episode','Playable production, payment conflict and ordinary control.'],['objects','Objects & systems','Relationships, operating rules and the economy.'],['narrative','Narrative','Chapter sequence, events and characters.'],['interface','Interface','Views, desk layouts and evidence displays.'],['art','Art & sound','Pixel system, component kit, assembled scene and interface.'],['production','Build & test','Architecture, source review and decision gates.']];
const gdd=read('GDD.md');
const phaseBlock=gdd.match(/<!-- production:table -->([\s\S]*?)<!-- \/production:table -->/);
if(!phaseBlock)throw new Error('GDD production table missing');
const phases=phaseBlock[1].trim().split('\n').slice(2).map(line=>{
 const [phase,status,output,gate]=line.split('|').slice(1,-1).map(s=>s.trim());
 const [id,title]=phase.split(' · ');
 if(!id||!title||!status||!output||!gate)throw new Error('Incomplete GDD phase: '+line);
 return {id,title,status,output,gate};
});
if(phases.map(p=>p.id).join(',')!=='P0,P1,P2,P3,P4,P5,P6,P7')throw new Error('GDD phases must retain stable P0–P7 identities');
if(!phases.find(p=>p.id===project.focus.phase)?.status.startsWith('Ready for playtest'))throw new Error('Project focus conflicts with GDD phase status');
figures['production-plan']=productionPlan(phases);
const chapters=gdd.replace(phaseBlock[0],'').split(/^## \d+\. /m).slice(1);
...
const routesByFile={'GDD.md':'game','evidence-map.md':'evidence','research/systems.md':'research-systems','research/ux.md':'research-ux','research/sprite-refinement.md':'research-refinement','research/office-art-research.md':'research-offices','research/campus-discoveries.md':'research-discoveries','research/park-logic-review.md':'research-park','binding/episode-01-draft.md':'binding','binding/native-inventory.md':'binding-inventory'};
const allIds=new Set();
for(const p of pages){
 const headings=[];
 let html=marked.parse(p.markdown);
 html=html.replace(/<!-- layout:(campaign|events) -->([\s\S]*?)<!-- \/layout:\1 -->/g,(_,kind,body)=>readingLayout(body,kind));
 ...
 html=html.replace(/<p>(<img[^>]+>)<\/p>/g,(_,img)=>{
  ...
  const catalogue=src.startsWith('assets/sprite-catalogue/');
  const href=catalogue?'assets/sprite-catalogue/index.html'+(object?'#'+object:''):src;
  const record=object&&catalogueData.objects.find(o=>o.id===object),animated=record?.display.animated;
  return `<figure class="inline-example"><a href="${href}" ${animated?'class="inline-object-motion"':''}>${animated?objectMedia(object,{title:alt,width:230,height:145}):img}</a><figcaption>${alt}<a href="${href}">${object?'Inspect '+object:catalogue?'Inspect catalogue':'Open source image'} ↗</a>${animated?playbackButton:''}</figcaption></figure>`;
 });
...
const contents=`<section class="contents-page" id="page-contents"><aside class="chapter-notes index-notes"><span class="note-id">GDD / 17 SEPTEMBER 2026</span><h1>Design document</h1><p class="index-intro">Game structure, narrative, interface and production plan.</p>${sourceCard(false)}${readerPreviewLinks}<p class="scope-note">The reconstruction and selected first-episode exports have been inspected. Original statements and key identity links remain open. <a href="#evidence/what-has-actually-been-read">Source scope ↗</a></p></aside><div class="contents-main"><div class="index-label"><h2>Contents</h2><span>08 CHAPTERS</span></div><div class="contents-rows">${pages.slice(0,8).map(p=>`<a class="contents-row" href="#${p.id}"><span>${p.number}</span><h2>${esc(p.title)}</h2><p>${p.summary}</p><span aria-hidden="true">↗</span></a>`).join('')}</div><div class="reference-index"><div><h2>Research & source work</h2></div><ul>${pages.slice(8).map(p=>`<li><a href="#${p.id}">${p.title}<span aria-hidden="true">↗</span></a></li>`).join('')}</ul></div></div></section>`;
const sections=pages.map((p,i)=>`<section class="reader-page" id="page-${p.id}" hidden aria-labelledby="title-${p.id}"><aside class="chapter-notes"><div class="note-id">${i<8?'CHAPTER':'REFERENCE'} / ${p.number}</div><header class="chapter-heading"><h1 id="title-${p.id}" tabindex="-1">${esc(p.title)}</h1></header>${p.intro?`<div class="${i<7?'chapter-intro':'source-preface'}">${p.intro}</div>`:''}<nav class="page-links" aria-label="In ${esc(p.title)}">${p.headings.filter(x=>x.level===2).map((h,i)=>`<a href="#${h.id}"><span>${String(i+1).padStart(2,'0')}</span>${esc(h.title)}</a>`).join('')}</nav>${[1,7].includes(i)?sourceCard(i===7):`<div class="release-note">DESIGN ${project.designRevision} / ART ${project.artRevision}<br><a href="previews/index.html">Current previews ↗</a><br><a href="assets/sprite-catalogue/index.html">Object catalogue ↗</a><br><a href="archive/index.html">Archive ↗</a></div>`}${i>=7?`<div class="document-tools"><a href="${p.file}">Editable source ↗</a></div>`:''}</aside><div class="chapter-main"><article class="article-body">${i>=8?'<p class="reference-status">Supporting research and binding notes. See the main chapters for current implementation and next steps. Each claim retains its source and verification status.</p>':''}${p.body}</article><nav class="chapter-footer" aria-label="Chapter navigation"><a href="#${i===0?'contents':pages[i-1].id}"><small>Previous</small>${i===0?'Contents':esc(pages[i-1].title)}</a><a href="#${i===pages.length-1?'contents':pages[i+1].id}"><small>${i===pages.length-1?'Return to':'Next'}</small>${i===pages.length-1?'Contents':esc(pages[i+1].title)} →</a></nav></div></section>`).join('');
const metadata=pages.map(({id,title,number,headings,file})=>({id,title,number,headings,file}));
...
${read('working/editorial.css')}\n${read('working/reading-layout.css')}</style></head><body><a class="skip" href="#main">Skip to document</a><header class="masthead"><a class="brand" href="#contents" aria-label="Mad Money Tycoon — contents"><span class="brand-name">Mad Money Tycoon</span></a><div class="header-context"><span>GAME DESIGN</span><i>/</i><b id="current-chapter">Contents</b><span id="current-number"></span></div><nav class="masthead-actions" aria-label="Reader navigation"><button id="return-to-origin" hidden aria-label="Return to previous section">← <span>Return</span></button><a href="#contents" class="header-contents">Contents</a><a href="whiteboard/index.html" class="header-whiteboard" title="Open the live design whiteboard"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 3h16v14H4zM9 17l-3 5m9-5 3 5M7 12l3-4 3 3 4-5" fill="none" stroke="currentColor" stroke-width="1.5"/></svg><span>Board</span></a><a href="basecamp/index.html" class="header-basecamp" aria-label="Basecamp (WIP)" title="Game concepts and prototypes · work in progress"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 5h7l3 3h8v12H3zM3 10h18" fill="none" stroke="currentColor" stroke-width="1.5"/></svg><span>Basecamp</span><small>WIP</small></a><a href="previews/index.html" class="header-library">Library ↗</a><button class="search-button" id="open-search">Find <kbd>⌘ K</kbd></button><a id="header-prev" href="#contents" aria-label="Previous chapter">←</a><span id="header-position">00 / 08</span><a id="header-next" href="#game" aria-label="Next chapter">→</a></nav></header><div class="shell"><main id="main">${contents}${sections}</main></div><dialog class="search-dialog" id="search-dialog" aria-label="Find in document"><div class="search-top"><label for="search-input">Find in document</label><input id="search-input" type="search" placeholder="Try payment, authority, E-01…" autocomplete="off"><button class="search-close" id="close-search" aria-label="Close search">Esc</button></div><div class="search-results" id="search-results" aria-live="polite"></div></dialog><script>${script}</script><script>${read('working/pixel-kit.js')}</script><script>${read('working/park-kit.js')}</script><script>${read('working/hero-scene.js')}</script><script>window.MMTObjectDisplays=${JSON.stringify(Object.fromEntries(catalogue.objects.map(o=>[o.id,{...o.display,poster:'assets/sprite-catalogue/'+o.display.poster,gif:'assets/sprite-catalogue/'+o.display.gif}]))).replaceAll('<','\\u003c')};</script><script>${read('working/art-system.js')}</script><script>${read('working/reading-moments.js')}</script><script>${read('assets/sprite-catalogue/motion-player.js')}</script><script>${read('working/document-materials.js')}</script><script>${fs.existsSync(path.join(root,'working/office-pixel-study.js'))?read('working/office-pixel-study.js'):''}</script><script>${read('working/office-study-controls.js')}</script><noscript><style>.reader-page[hidden]{display:grid!important}.search-button{display:none}.reader-page{margin-top:80px}.chapter-notes{position:static}</style></noscript></body></html>`;
fs.writeFileSync(path.join(root,'design-review.html'),html);
fs.writeFileSync(path.join(root,'working/production-plan.json'),JSON.stringify({source:'GDD.md#production-sequence',designRevision:project.designRevision,phases},null,2)+'\n');
