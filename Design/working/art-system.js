...
function render(){
 ...
 const display=window.MMTObjectDisplays[item.id],sprite=l?display.poster:'assets/sprite-catalogue/objects/'+item.id+'/'+key+'.png';
 body.innerHTML=`<a class="park-selected-sprite" href="assets/sprite-catalogue/index.html#${item.id}"><img src="${sprite}" ${l&&display.animated?`data-motion-src="${display.gif}"`:""} alt="${item.name}, ${stateName.toLowerCase()}"><span>Object sheet & GIF downloads ↗</span></a>${l&&display.animated?'<button type="button" data-preview-motion>Pause previews</button>':''}<a class="park-source-thumb" href="${details.photo}" target="_blank" rel="noopener"><img src="${details.photo}" alt="Source photograph for ${item.name}"><span>Source photograph ↗</span></a><dl><dt>Source</dt><dd>${details.observed}</dd><dt>Park interpretation</dt><dd>${details.invented}</dd></dl>${l&&l.id==='L-05'?'<a href="previews/voila-delivery.gif" download>Download delivery loop GIF ↓</a>':''}<a class="park-source-link" href="${item.source}" ${item.source.startsWith('http')?'target="_blank" rel="noopener"':''}>${l?(l.id==='L-05'?'Component and motion →':'Source project ↗'):'Architecture references →'}</a>`;
